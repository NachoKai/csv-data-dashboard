...
export function useIndexedDB() {
  async function load(): Promise<WorkspaceDataset[]> {
    try {
      ...
      const browserLegacy = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem(LEGACY_KEY) || 'null') : null
      return datasetFromLegacy(browserLegacy)
    } catch {
      try { return datasetFromLegacy(JSON.parse(localStorage.getItem(KEY) || localStorage.getItem(LEGACY_KEY) || 'null')) } catch { return datasetFromLegacy(null) }
    }
  }
  async function save(value: WorkspaceDataset[]) {
    try { await set(KEY, value) } catch { localStorage.setItem(KEY, JSON.stringify(value)) }
  }
  return { load, save }
}
